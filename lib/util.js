...
const max = 1000; // ランダムのとってくる数の最大値+1。randomNumbersGet25np関数内のmaxNowに渡して可変にする。増加幅は同じ関数内のdで設定。
const kosuu = 30; // ランダムにとってくる数の個数
...
function randomNumbersGet25np() {
  ...
  while (npMap.size + kosuu > maxNow * 0.2) { // 「2,5の倍数でない数」はmaxNow * 0.4 あるが，この中には素数も含まれていることに注意
